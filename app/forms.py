...
from wtforms import BooleanField, DateField, FloatField, SelectField, StringField
...
class UpdateLimit(FlaskForm):
    new_limit = FloatField(
        "Новый лимит:", validators=[DataRequired(message="Введите лимит")]
    )


class Settings(FlaskForm):
    default_limit = FloatField(
        "Лимит по умолчанию:",
        validators=[DataRequired(message="Введите новый лимит")],
        default=config["default_limit"],
    )
    adaptive_limit = BooleanField(
        "Адаптивный лимит",
...
class CustomLimit(FlaskForm):
    first_month = StringField(
        "Начало периода:",
        validators=[
            DataRequired(message="Введите начало периода"),
            Regexp("^\d{4}-[0-1]?\d$", message="Введите дату в формате ГГГГ-ММ"),
        ],
    )
    last_month = StringField(
        "Конец периода (не обязательно):",
        validators=[
            Regexp(
                "^(?:\d{4}-[0-1]?\d|[\s]*)$", message="Введите дату в формате ГГГГ-ММ"
            )
        ],
    )
